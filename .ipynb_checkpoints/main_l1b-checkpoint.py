...
from ffsar_processing import OmegakProcessing, BackprojectionProcessing
from cst import Cst
import utils
...
if float(config.get('PROCESSING_OPTIONS','dt_update_h'))>=0.06 and float(config.get('PROCESSING_OPTIONS','dt_update_h'))<=2.3:
    dt_update_h=float(config.get('PROCESSING_OPTIONS','dt_update_h'))
else:
    logging.error("Uncorrect dt_update_h, must be between 0.06 and 2.3")
    sys.exit(0)
...
n_fft_az=int(illumination_time/cst.PRI)

data_buffer=DataBuffer(file_l1a,n_burst_block)
data_block=DataBlock(n_burst_block,data_buffer,gprw_mean)

if l1b_processing=="ffsar_omegak":
    processing=OmegakProcessing(data_block,n_fft_az,n_sl_output=n_sl_output,n_multilook=n_multilook,hamming_az=hamming_az,hamming_range=hamming_range,zp=zp,range_ext_factor=range_ext_factor)
    logging.info("Using the FF-SAR omega-k processing")
elif l1b_processing=="ffsar_bp":
...
if l1b_processing=="ffsar_omegak":
    file_l1b.dt_update_h=dt_update_h
